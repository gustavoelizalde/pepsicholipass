...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('contenidos-parametros-grid', {
		data: $(this).serialize()
	});
	return false;
});
        ...
        <div class="box-wrap clear">
            ...
            <div class="clear" style="margin-bottom:20px;">
                ...
                <div class="clear">
                    ...
                    <?php if (Yii::app()->user->checkAccess('crearParametros') || Yii::app()->user->id == 'admin-root') { ?>
                        <?php echo CHtml::link('Crear Nuevo', array('contenidosparametros/create'), array('class' => 'button blue fl')); ?>
                    <?php } ?>
                </div>
            ...
            <?php
            if ((Yii::app()->user->checkAccess('editarParametros') && Yii::app()->user->checkAccess('eliminarParametros')) || Yii::app()->user->id == 'admin-root')
                $template = '{update}{delete}';
            elseif (Yii::app()->user->checkAccess('editarParametros'))
                $template = '{update}';
            elseif (Yii::app()->user->checkAccess('eliminarParametros'))
                $template = '{delete}';
            else
                $template = '';

            $this->widget('zii.widgets.grid.CGridView', array(
                'id' => 'contenidos-parametros-grid',
